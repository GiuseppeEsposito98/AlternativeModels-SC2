...
def get_argparser():
    ...
    # parser.add_argument('--config', required=True, help='yaml file path', default='/home/g.esposito/sc2-benchmark/configs/coco2017/supervised_compression/ghnd-bq/faster_rcnn_resnet50-bq1ch_fpn_from_faster_rcnn_resnet50_fpn.yaml')
    ...
    parser.add_argument('-train_teacher', action='store_true', help='train first the teacher model')
...
def evaluate(model_wo_ddp, data_loader, iou_types, device, device_ids, distributed, no_dp_eval=False,
    ...
    for sample_batch, targets in metric_logger.log_every(data_loader, log_freq, header):
        ...
        # i think that the error is only here because i have discovered that the inference is computed on the whole model
        # but the training is computed only in the trainable layers
...
def main(args):
    ...
    if not args.student_only and teacher_model is not None:
        ...
        for i in [1,3,6,8,12,14,16,19]:
            exec(f'student_model.backbone.features.body.bottleneck_layer.max{i}.clear()')
            evaluate(student_model, train_data_loader, iou_types, device, device_ids, distributed,
                    no_dp_eval=no_dp_eval, log_freq=log_freq, header='Validation:', writer=writer)
            exec(f'logger.info(max(student_model.backbone.features.body.bottleneck_layer.max{i}))')
