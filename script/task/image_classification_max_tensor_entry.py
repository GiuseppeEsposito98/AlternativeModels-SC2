...
def main(args):
    ...
    if not args.student_only and teacher_model is not None:
        ...
        for i in [2,5,8,11,14]:
            exec(f'student_model.features.bottleneck_layer.max{i}.clear()')
        evaluate(student_model, train_data_loader, device, device_ids, distributed,
                    no_dp_eval=no_dp_eval, log_freq=log_freq, header='Validation:')
        for i in [2,5,8,11,14]:
            exec(f'logger.info(max(student_model.features.bottleneck_layer.max{i}))')
