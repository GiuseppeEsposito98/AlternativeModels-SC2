...
def fix_model(student_model, bounding_ranges):
    ...
    print(student_model)
    ranger_student = ModifiedStudent(student_model, bounding_ranges)
    # print(ranger_student)
    ...
    for key in list(state_dict.keys()):
        if key.startswith('features.bottleneck_layer.encoder'):
            fields = key.split('.')
            if fields[3] == '4':
                k_new = 'features.bottleneck_layer.encoder.6.'+ fields[4]
                state_dict[k_new] = state_dict.pop(key)
            if fields[3] == '3':
                k_new = 'features.bottleneck_layer.encoder.5.'+ fields[4]
                state_dict[k_new] = state_dict.pop(key)
    # print(state_dict['features.bottleneck_layer.encoder'].keys())
    ranger_student.weak_model.load_state_dict(state_dict)
    return ranger_student
    

class ModifiedStudent(torch.nn.Module):
    def __init__(self, pretrained_model, bounding_ranges):
        super(ModifiedStudent, self).__init__()
        self.bounding_act = bounding_ranges

        # Clone the pretrained model
        self.weak_model = deepcopy(pretrained_model)

        # Modify the convolutional block
        self.modify_conv_block()

    def modify_conv_block(self):
        act_counter = 0
        current_max = self.bounding_act[act_counter]
        tmp = list(self.weak_model.features.bottleneck_layer.encoder.children())
        void = list()
        for el in tmp:
            void.append(el)
            if not isinstance(el, torch.nn.Conv2d) and not isinstance(el, torch.nn.Hardtanh):
                void.append(torch.nn.Hardtanh(min_val=0, max_val=current_max))
            elif isinstance(el, torch.nn.ReLU):
                act_counter+=1
        tmp_seq = torch.nn.Sequential(*void)
        self.weak_model.features.bottleneck_layer.encoder = tmp_seq
...
def main(args):
    ...
    train_batch_size=config['train']['train_data_loader']['batch_size']
    train_shuffle=config['train']['train_data_loader']['random_sample']
    train_num_workers=config['train']['train_data_loader']['num_workers']
    train_data_loader_config = config['train']['train_data_loader']
    ...
    # print(int(0.2*len(train_data_loader))/100)
    # 25 imgs per class
    subsampler = DatasetSampling(train_data_loader.dataset,25)
    index_dataset=subsampler.listindex()
    data_subset=Subset(train_data_loader.dataset, index_dataset)
    subset_dataloader = DataLoader(data_subset,batch_size=train_batch_size, shuffle=train_shuffle,pin_memory=True,num_workers=train_num_workers)
